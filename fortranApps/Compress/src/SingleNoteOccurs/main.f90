...
program hello
    ...
    do num = 1, fileLen, 1
       ...
       if (readHere(1:1) == "-") then
            notes(num2)%length =  num3-1
            notes(num2)%bytes = "                "

            do num4 = 1, num3-1, 1
               notes(num2)%bytes(num4) = tempShit(num4)
            end do
            ...
            if (num2 > 1) then
                do num4 = 1, num2-1, 1
                   do num5 = 1, num3-1, 1
                      if (notes(num4)%bytes(num5) /= tempShit(num5)) exit

                      if (num5 == num3 - 1) existing = .TRUE.
                   end do
                end do
            end if
    ...
    do num = 1, last, 1
       ...
       do num2 = 1, fileLen-notes(num)%length, 1

          do num3 = 1, notes(num)%length, 1

             if (notes(num)%bytes(num3) /= bytes(num2+num3)) exit
             if ((num2+num3) == fileLen) then
                notes(num)%occurence = notes(num)%occurence + 1
                exit
             end if

             if (bytes(num2+num3+1)(1:3) == "---") then
                notes(num)%occurence = notes(num)%occurence + 1
             end if
