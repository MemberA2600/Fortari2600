...
program Finalizing
    ...
    type(keyWeight), dimension(13) :: keyWeights
    type(KeyPattern), dimension(13) :: keyPatterns
    ...
    open(unit=32, file = tempPath // "Input02.txt")

    do num = 1, 13, 1
       read(32, *) keyWeights(num)%key, keyWeights(num)%weight
    end do
    ...
    do num = 1, 13, 1
       ...
       keyPatterns(num)%title = title // "_Channel" // dummy // "_" // keyList(num)
       keyPatterns(num)%key = keyList(num)
       keyPatterns(num)%length = patterns(keyWeights(num)%key)%length

       keyPatterns(num)%bytes = "        "

       do num2 = 1, keyPatterns(num)%length, 1
          keyPatterns(num)%bytes(num2) = patterns(keyWeights(num)%key)%bytes(num2)
       end do

    end do

    open(unit = 13, file = tempPath // "Output00.txt", ACTION = "write")
    do num = 1, 13, 1
        write(13, "(A)") trim(keyPatterns(num)%title)
            do num2 = 1, keyPatterns(num)%length, 1
               if (keyPatterns(num)%bytes(num2) /= "---     ") then
                   write(13, "(A)") achar(9) // "BYTE" // achar(9) // "#%" // keyPatterns(num)%bytes(num2)
               end if
            end do
        write(13, "(A)") achar(9) // "BYTE" // achar(9) // "#%11100000"
        if (num /= 13) write(13, "(A)") ""
    end do
    close(13)
    ...
    do num = 1, 13, 1
       ...
       open(unit = 13, file = tempPath // "Output" // fileNum // ".txt")
        do num2 = 1, keyPatterns(num)%length, 1
           if (keyPatterns(num)%bytes(num2)(1:3) == "---") then
                write(13, "(A)") "---"
           else
                write(13, "(A)") achar(9) // "BYTE" // achar(9) // "#%" // keyPatterns(num)%bytes(num2)
            end if
